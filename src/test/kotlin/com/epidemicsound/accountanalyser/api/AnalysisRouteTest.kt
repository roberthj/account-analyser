...
class AnalysisRouteTest {
    ...
    @Test
    fun `POST analyse returns 200 and a verdict for a valid request`() = testApplication {
        application { module() }
    ...
    @Test
    fun `POST analyse returns 400 for invalid significance level`() = testApplication {
        application { module() }
        val client = createClient {
            install(ContentNegotiation) { json() }
        }
